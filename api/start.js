...
// Resumes a suspended service on the given account (1-10).
...
export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(`https://api.render.com/v1/services/${serviceId}/resume`, {
      method: 'POST',
      headers: {
    ...
    if (r.status === 202) {
      return res.status(200).json({ ok: true, serviceId, status: 'resuming' });
    }

    const body = await r.json().catch(() => ({}));
    return res.status(r.status).json({ ok: false, error: body.message || `Render API ${r.status}` });
  } catch (err) {
    return res.status(500).json({ error: err.message || 'Failed to start service' });
  }
}
